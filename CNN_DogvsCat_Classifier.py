...
train_data = Preprocessor.read_model_sets(train_path, img_size, classes)
test_data = Preprocessor.read_test_sets(test_path, img_size, classes)
val_data = Preprocessor.read_model_sets(val_path, img_size, classes)

print("Size of:")
print("  - Training-set:\t\t{}".format(len(train_data.train.labels)))
print("  - Test-set:\t\t{}".format(len(test_data.train.labels)))
print("  - Validation-set:\t{}".format(len(val_data.valid.labels)))
...
def optimize(num_iterations):
    ...
    for i in range(total_iterations, total_iterations + num_iterations):
        ...
        x_batch, y_true_batch, _, cls_batch = train_data.train.next_batch(batch_size)
        x_valid_batch, y_valid_batch, _, valid_cls_batch = val_data.train.next_batch(batch_size)
...
def plot_example_errors(cls_pred, correct):
    ...
    images = val_data.train.images[incorrect]
    
    # Get the predicted classes for those images.
    cls_pred = cls_pred[incorrect]

    # Get the true classes for those images.
    cls_true = val_data.train.cls[incorrect]
...
def print_validation_accuracy(show_example_errors=False, show_confusion_matrix=False):
    ...
    num_test = len(val_data.train.images)
    ...
    while i < num_test:
        ...
        images = val_data.train.images[i:j, :].reshape(batch_size, img_size_flat)
        

        # Get the associated labels.
        labels = val_data.train.labels[i:j, :]
    ...
    cls_true = np.array(val_data.train.cls)
...
plt.axis('off')
test_NORMAL = cv2.imread('Test_image/NORMAL.2.jpeg', cv2.IMREAD_GRAYSCALE)
...
test_PNEUMONIA = cv2.imread('Test_image/PNEUMONIA.6.jpeg', cv2.IMREAD_GRAYSCALE)
test_PNEUMONIA = cv2.resize(test_PNEUMONIA, (img_size, img_size), cv2.INTER_LINEAR) / 255
preview_dog = plt.imshow(test_PNEUMONIA.reshape(img_size, img_size, num_channels))
...
print("Predicted class for test_PNEUMONTIA: {}".format(sample_prediction(test_PNEUMONIA)))
